...
//
	...
	for (var i=0; i < el.childNodes.length; i++)
	{
		...
		{
			if(child.className != "treeUnselectedCaption" && child.className != "treeSelectedCaption")
				child.className = className;
			else
				if(className == 'treeSelected')
					child.className = 'treeSelectedCaption';
				else
					child.className = 'treeUnselectedCaption';
			break;
		}
	}
}
...
//
	...
	for(var i=0; i < el.childNodes.length; i++)
	{
		...
		else if (child.className == "treeSubnodesHidden")
		{
			...
			lnk.className="treeUnselectedCaption";
			break;
		}
...
function OpenAll()
{
	var treeElement = document.getElementsByTagName('div');
	
	for (var i = 0; i < treeElement.length; i++)
	{
		if (treeElement[i].className == 'treeNode')
		{
			expandNode(treeElement[i]);
		}
	}
	treeElement = null;
}
...
function resizeTree()
{
	...
	{
		if(document.body.clientHeight - 27>0)
			treeDiv.style.height = document.body.clientHeight - 27;
	}
	else
	{
		if(document.documentElement.offsetHeight - 27>0)

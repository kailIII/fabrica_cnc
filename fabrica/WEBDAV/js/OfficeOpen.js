OfficeOpen=(function(){if(!Array.prototype.indexOf){Array.prototype.indexOf=function(p,o){if(o==null){o=0
}else{if(o<0){o=Math.max(0,this.length+o)
}}for(var n=o,m=this.length;
n<m;
n++){if(this[n]===p){return n
}}return -1
}
}var e=false,i=null,h=0;
function l(m){if(typeof m=="undefined"){return false
}var n=String(m);
return n.substring(n.lastIndexOf(".")+1,n.length).toLowerCase()
}function a(m){var n=l(m);
if(OfficeOpen.wordTypes.split(",").indexOf(n)>=0){return 0
}if(OfficeOpen.excelTypes.split(",").indexOf(n)>=0){return 1
}if(OfficeOpen.outlookTypes.split(",").indexOf(n)>=0){return 2
}if(OfficeOpen.powerTypes.split(",").indexOf(n)>=0){return 3
}if(OfficeOpen.accessTypes.split(",").indexOf(n)>=0){return 4
}if(OfficeOpen.frontPageTypes.split(",").indexOf(n)>=0){return 5
}if(OfficeOpen.publisherTypes.split(",").indexOf(n)>=0){return 6
}return -1
}function b(n){n=String(n||"");
var m=n.length;
if(m>0){var q=n.indexOf("/",8);
if(q>=0&&q<m-1){var p=n.substring(q+1,m).split("/");
for(var o=0;
o<p.length;
o++){p[o]=window.encodeURIComponent(p[o])
}p.unshift(n.substring(0,q));
return p.join("/")
}}return n
}var f=new (function(){try{this.firefox36up=false;
this.firefox35up=false;
var o=navigator.userAgent.toLowerCase();
this.osver=1;
if(o){var n=o.substring(o.indexOf("windows ")+11);
...
this.nav7up=false;
if(this.nav6up){var s=o.indexOf("netscape/");
if(s>=0){this.nav7up=parseInt(o.substring(s+9))>=7
}}this.ie=(o.indexOf("msie")!=-1);
this.aol=this.ie&&o.indexOf(" aol ")!=-1;
if(this.ie){var r=o.substring(o.indexOf("msie ")+5);
...
if(this.safari&&this.major>=5){var q=o.indexOf("webkit/");
if(q>=0){this.safari125up=parseInt(o.substring(q+7))>=125
}var w=o.indexOf("version/");
if(w>=0){this.safari3up=parseInt(o.substring(w+8))>=3
}}this.firefox=this.nav&&(o.indexOf("firefox")!=-1);
this.firefox3up=false;
this.firefox35up=false;
this.firefox36up=false;
this.firefox4up=false;
if(this.firefox&&this.major>=5){var B=o.indexOf("firefox/");
if(B>=0){var A=o.substring(B+8);
var y=parseInt(A);
this.firefox3up=y>=3;
this.firefox4up=y>=4;
var u=parseFloat(A);
...
if(this.chrome){var z=o.indexOf("chrome/");
if(z>=0){var t=o.substring(z+7);
var p=parseInt(t);
this.chrome2up=p>=2;
this.chrome3up=p>=3;
this.chrome6up=p>=6
...
if(this.opera){var m=o.indexOf("version/");
if(m>=0){var v=o.substring(m+8);
this.opera11up=parseFloat(v)>=11
}}this.yandex=this.nav&&(o.indexOf("YaBrowser")!=-1)
}catch(x){}})();
function d(){var s=function(){return f.mac&&(f.firefox3up||f.safari3up)
};
var v=function(x){return navigator.mimeTypes&&navigator.mimeTypes[x]&&navigator.mimeTypes[x].enabledPlugin
};
var n=function(){var x=v("application/x-sharepoint-webkit");
var y=v("application/x-sharepoint");
if(f.safari3up&&x){return true
}return y
};
var m=function(){var y=null;
if(s()){y=document.getElementById("macSharePointPlugin");
if(y==null&&n()){var x=null;
if(f.safari3up&&v("application/x-sharepoint-webkit")){x="application/x-sharepoint-webkit"
}else{x="application/x-sharepoint"
...
var r=function(){return(f.winnt||f.win32||f.win64bit)&&f.firefox3up
};
var q=function(){var x=null;
if(r()){try{x=document.getElementById("winFirefoxPlugin");
if(!x&&v("application/x-sharepoint")){var z=document.createElement("object");
...
var w=null;
if(window.ActiveXObject){var o=[function(){return new ActiveXObject("SharePoint.OpenDocuments.5")
...
++p){try{w=(o[p])();
break
}catch(u){}}}else{if(f){try{if(s()){w=m()
}else{if(r()){w=q()
}}}catch(u){}}}return w
}function c(q){var s=String(q).replace(/%/g,"%25").replace(/&/g,"%26").replace(/#/g,"%23").replace(/\+/g,"%2B");
var n=d();
if(!n&&window.ActiveXObject){var p=l(q);
if(!p){return false
}p=","+p+",";
var m;
if((","+OfficeOpen.wordTypes+",").indexOf(p)>-1){m=activeXMSOfficeApplications.Word
}else{if((","+OfficeOpen.excelTypes+",").indexOf(p)>-1){m=activeXMSOfficeApplications.Excel
}else{if((","+OfficeOpen.powerTypes+",").indexOf(p)>-1){m=activeXMSOfficeApplications.PowerPoint
}}}if(typeof m!="undefined"){try{var o=new ActiveXObject(m.app);
if(typeof o.Visible!="undefined"){o.Visible=true
}else{if(typeof o.ActiveWindow!="undefined"){o.ActiveWindow.Visible=true
}}if(typeof m.obj!="undefined"){o[m.obj].Open(s);
return true
}}catch(r){return false
}}return false
}try{n.EditDocument(s);
return true
}catch(r){return false
}}function k(n){if(window.ActiveXObject){try{var r=new ActiveXObject("com.sun.star.ServiceManager");
if(r){var q=r.createInstance("com.sun.star.frame.Desktop");
if(q){var o=r.Bridge_GetStruct("com.sun.star.beans.PropertyValue");
...
}function g(m,n){switch(n){case 0:if(!c(m)){return k(m)
}return true;
case 1:if(!k(m)){return c(m)
}return true;
case 2:return c(m);
case 3:return k(m);
default:return false
}}function j(n,r){if(!g(n,r)){var p=document.getElementById("OfficeLauncher");
if(!p){i=n;
h=r;

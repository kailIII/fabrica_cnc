...
class Sabre_DAV_Locks_Plugin extends Sabre_DAV_ServerPlugin {
    ...
    protected function httpUnlock($uri) {
		...
		//Modified by Crush . Added || 'opaquelocktoken:' . $lock->token == $lockToken 
		//to support microsoft webdav miniredir
			if ('<opaquelocktoken:' . $lock->token . '>' == $lockToken || 'opaquelocktoken:' . $lock->token == $lockToken) {

...
class Sabre_DAV_Server {
    ...
    public function guessBaseUri() {
        ...
        if (!empty($pathInfo)) {
            ...
            throw new Sabre_DAV_Exception('The REQUEST_URI ('. $decodedUri . ') did not end with the contents of PATH_INFO (' . $pathInfo . '). This server might be misconfigured.'); 
    ...
    protected function httpOptions($uri) {
		...
		$this->httpResponse->setHeader('Public',strtoupper(implode(', ',$methods)));
    ...
    protected function httpGet($uri) {
		...
		if (!isset($httpHeaders['Content-Disposition'])) 	
		{
			 list(, $name) = Sabre_DAV_URLUtil::splitPath($uri);
			 $httpHeaders['Content-Disposition'] = 'attachment; filename="'.$name.'"';
		}
    ...
    protected function httpHead($uri) {
		...
		{			
			//echo $this->httpResponse->setHeader("Content-Length", 1230, true);
			$this->httpResponse->sendStatus(405);	
		}
    ...
    protected function httpPropfind($uri) {
		...
		$this->httpResponse->setHeader('Content-Length',strlen($data));		
    ...
    protected function httpMkcol($uri) {
        ...
        if ($requestBody) {
            ...
            if (strpos($contentType,'text/xml')!==0 && strpos($contentType,'text/xml')!==0) {
            ...
            foreach($propertyNames as $prop) {
                ...
                switch($prop) {
					...
					case '{DAV:}displayname':
						$newProperties[200][$prop] = $node->getName(); break;
					/*case '{DAV:}IsReadOnly' :						
						$newProperties[200][$prop] = 0; break;*/
                    case '{DAV:}getetag'               : if ($node instanceof Sabre_DAV_IFile && $etag = $node->getETag())  $newProperties[200][$prop] = $etag; break;
                    case '{DAV:}getcontenttype'        : if ($node instanceof Sabre_DAV_IFile && $ct = $node->getContentType())  $newProperties[200][$prop] = $ct; 
														if ($node instanceof Sabre_DAV_ICollection)  $newProperties[200][$prop] = "";//"application/octet-stream"; 
														break;
				...
				if (!$allProperties && !isset($newProperties[200][$prop]) && $prop != "href" && $prop != "{DAV:}href") $newProperties[404][$prop] = null;

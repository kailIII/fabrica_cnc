<?
//----
$sql = "SELECT A.id_metodologia,A.nom_metodologia
 FROM ".tablaMetodologia." A INNER JOIN ".tablaMetodologiaRTA." B USING (id_metodologia)
  WHERE id_propuesta=".$idPropuesta."
   ORDER BY A.id_metodologia";
//echo '<BR>'.$sql;
$filasProcesos			= NULL;
$con					= mysql_query($sql);
while($campos			= mysql_fetch_array($con)){
	$id_metodologia		= $campos["id_metodologia"];
	$nom_metodologia	= $campos["nom_metodologia"];

	$filasProcesos	.= "
	<TABLE width='98%' border='0' cellspacing='0' cellpadding='0' align='center'>
	 <TR>
	  <TD width='1%' align='right' class='bb divInstruccion'>&nbsp;</TD>
	  <TD width='40%' align='left' class='bb divInstruccion'><div class='padding5'><B>Procesos / Tiempo en semanas</B></div></TD>
	  <TD width='20%' align='center' class='bb divInstruccion'><div class='padding5'><B>$nom_metodologia</B></div></TD>
	  <TD align='right' class='bb divInstruccion'>&nbsp;</TD>
	 </TR>";

	$sqlP = "SELECT * FROM ".tablaProceso." ORDER BY 1";
	//echo '<BR>'.$sqlP;
	$conP				= mysql_query($sqlP);
	while($camposP		= mysql_fetch_array($conP)){
		$id_proceso		= $camposP["id_proceso"];
		$nom_proceso	= $camposP["nom_proceso"];
	
		//---- consulta si el proceso aplica para la propuesta activa
		$sqlR = "SELECT * FROM ".tablaTiempoProcesoRTA."
		 WHERE id_propuesta=".$idPropuesta." AND id_metodologia=".$id_metodologia." AND id_proceso=".$id_proceso;
		//echo '<BR>'.$sqlR;
		$chObj				= NULL;
		$tiempoProceso		= NULL;
		$conR				= mysql_query($sqlR);
		while($camposR		= mysql_fetch_array($conR)){
			$tiempoProceso	= $camposR["tiempo"];
			$chObj			= "checked='checked'";
		}
		if($propuestaNueva == 1){
			$chObj			= "checked='checked'";
			//---- consulta el tiempo por defecto
			$sqlR = "SELECT * FROM ".tablaTiempoProceso."
			 WHERE id_metodologia=".$id_metodologia." AND id_proceso=".$id_proceso;
			//echo '<BR>'.$sqlR;
			$conR				= mysql_query($sqlR);
			while($camposR		= mysql_fetch_array($conR)){
				$tiempoProceso	= $camposR["tiempo"];
			}
		}
//		$nameObj	= 'm'.$id_metodologia.nameObjProcesos.'[]';
		$nameObj	= 'm'.$id_metodologia.'p'.$id_proceso;
		$idObj		= $nameObj;
		$objCH		= "<INPUT type='checkbox' name='$nameObj' id='$idObj' value='$id_proceso' $chObj />";

		$nameObj	= 'm'.$id_metodologia.'p'.$id_proceso.'tiempo';
		$idObj		= $nameObj;

		$objTiempo	= "<INPUT type='text' name='$nameObj' id='$idObj' maxlength='2' class='borderBlue' value='$tiempoProceso' style='width:40px; padding:3px; text-align:center;' onkeypress='return esNumero(event);' />";

		$filasProcesos		.= "
		 <TR>
		  <TD align='right' class='bb'><div>$objCH</div></TD>
		  <TD align='left' class='bb'><div class='padding5 textLabel'>$nom_proceso</div></TD>
		  <TD align='center' class='bb'><div class='padding5'>$objTiempo</div></TD>
		  <TD align='center' class='bb'>&nbsp;</TD>
		 </TR>";
	}
	$filasProcesos	.= "</TABLE>";
}
?>
	<TABLE width="100%" border="0" cellspacing="0" cellpadding="0" align="left" style='border:1px solid #CED7EC;'>
	 <TR>
	  <TD align="center" colspan="2">
	    <div class='padding5'><?=$filasProcesos?></div>
	  </TD>
	 </TR>
	</TABLE>
